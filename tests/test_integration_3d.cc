...
void test(std::string const &filename, std::string const &gold_filename_prefix,
          double const min_expected, double const max_expected)
{
  ...
  if (num_ranks == 1)
  {
    ...
    std::ofstream gold_file_writer(gold_filename_prefix + ".txt");
    ...
    */
    ...
    std::ifstream gold_file(gold_filename_prefix + ".txt");
  ...
  else if (num_ranks == 2)
  {
    ...
    /*
    // To write the gold file
    std::ofstream gold_file_writer(gold_filename_prefix + "_" +
    std::to_string(rank) +
                                   ".txt");
    for (unsigned int i = 0; i < temperature.locally_owned_size(); ++i)
      gold_file_writer << temperature.local_element(i) << " ";
    ...
    */
    ...
    std::ifstream gold_file(gold_filename_prefix + "_" + std::to_string(rank) +
                            ".txt");
...
{
  test("demo_316_short_anisotropic.info", "integration_3d_gold", 285.0, 500.0);
}

BOOST_AUTO_TEST_CASE(integration_3D_short, *utf::tolerance(0.1))
{
  test("demo_316_short.info", "integration_3d_gold_short", 285.0, 580.0);
}

BOOST_AUTO_TEST_CASE(integration_3D_five_axis, *utf::tolerance(0.1))
{
  test("five_axis.info", "integration_3d_gold_five_axis", 299.9, 5415.0);
}
